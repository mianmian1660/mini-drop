...
// 哨兵规则一行的展示文案：histogram 类信号（p50/p95/p99，微秒存储，前端按 ms 展示）。
...
export default function SentinelCard({ targetIP, signals = [] }) {
    ...
    const hasAnyEligible = eligibleSignals.length > 0;
...
function AddSentinelForm({ targetIP, signals, onCancel, onCreated }) {
    ...
    const [floor, setFloor] = useState('5'); // 阈值：ms
    const [cooldownMin, setCooldownMin] = useState(15);
    const [submitting, setSubmitting] = useState(false);
    const [error, setError] = useState('');

    const valid = signal && Number(cooldownMin) > 0 && Number(floor) > 0;
