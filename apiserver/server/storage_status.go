...
func (s *APIServer) StorageStatus(c *gin.Context) {
	...
	mode := s.pqModeOf()
	capacityHalted := s.capacityHalted()
	// StorageDiskSnapshot.CollectionAllowed 只表示 emergency/unknown 硬门禁。
	// 对外的 collection_allowed/new_collection_allowed 必须与真正的采集
	// 入口一致，同时反映动态 required_free 和恢复观察期。
	newCollectionAllowed := snap.CollectionAllowed && snap.AvailableBytes >= requiredFree && !capacityHalted
	// 阶段六：细粒度目录状态
	...
	s.RespondOK(c, gin.H{
		...
		"level":                  snap.Level,
		"collection_allowed":     newCollectionAllowed,
		"new_collection_allowed": newCollectionAllowed,
		...
		"capacity_halted":               capacityHalted,
		...
		"migration_receipt_count":            phase6.MigrationReceiptCount,
		"migration_receipt_gc_eligible":      phase6.MigrationReceiptGCEligible,
		"migration_receipt_gc_deleted_total": phase6.MigrationReceiptGCDeletedTotal,
