...
//
// 单一所有权：TimedProcessPoller 在 Attach() 时登记、每次 Poll() 时续租，
// 在真正收割进程（无论
// 是 Poll() 自然到终态、还是 ForceStop() 主动终止）时摘牌——这是它的常规
// 生命周期，不需要 CleanupWorker 插手。只有最后一次续租超过宽限期仍未
// 摘牌的条目，才是真正
...
namespace drop
{
    ...
    public:
        ...
        void Touch(pid_t pid);
        void Unregister(pid_t pid);

        /// 返回当前登记表的快照：(pid, 最后续租时刻)。CleanupWorker 用该时刻
