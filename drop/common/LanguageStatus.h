...
//   symbol_status / semantic_frame_percent / unresolved_frame_percent /
//   sample_count / reasons / processes
//
// 状态口径（阶段 4 冻结，服务端/前端不得再各自推导）：
//   collector_status:
//     ready    语义覆盖 >= 70% 且未解析率 <= 20% 且无确定性采集失败
...
struct LanguageProcessStatus
{
    ...
    std::string comm;
    std::string exe;
    std::string mode;    // perf-map | py-spy-native | goresym | perf-fp | perf-dwarf
