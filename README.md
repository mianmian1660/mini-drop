...
特别说明：`perf record` 的真实 CPU 采样依赖 Linux perf_event 和硬件 PMU/性能计数器。部分 VMware/VirtualBox/WSL 环境无法开启“虚拟化 CPU 性能计数器”，即使容器权限正确，也可能无法真跑 perf 硬件采样。这属于虚拟化环境限制，不是系统链路错误。正式演示建议使用可开启 PMU 的 Linux 虚拟机、Linux 裸机或云主机；如果当前机器无法开启 PMU，可以说明环境限制，并重点演示状态机、Agent 心跳、eBPF/pprof 或已有产物展示链路。
...
如果日志中出现 `mock`、`perf 不可用`、`eBPF 不可用`、`Operation not permitted`、`perf_event_open` 失败等信息，通常说明当前宿主机或虚拟机没有暴露所需内核能力。VMware 中“虚拟化 CPU 性能计数器”无法勾选时，CPU perf 真采大概率无法完成，应更换可用环境或在演示中明确说明。
...
如果 `make demo-cpu` 在 VMware 中无法生成真实 perf 火焰图，请先确认虚拟机是否支持虚拟化 CPU 性能计数器。若该选项无法勾选，建议不要把它作为机器故障处理，而是在演示说明中标注“当前 VMware 环境无法暴露 PMU，perf 真采需要换可用 Linux 环境”。系统仍可用于验证任务下发、状态迁移、Agent 心跳、产物展示和 eBPF/pprof 等其他链路。
