...
[[ "${AGENT_IDLE_TIMEOUT}" =~ ^[0-9]+$ && "${AGENT_IDLE_INTERVAL}" =~ ^[0-9]+$ && \
  ...
  || die CONFIG "Agent 空档门禁参数必须是非负整数" 2
...
REMOTE_ENV="$(printf 'REMOTE_PATH=%q DEPLOY_BRANCH=%q TARGET_SHA=%q SOURCE_REMOTE_URL=%q FETCH_TIMEOUT=%q TEST_SCOPE=%q SERVICE_SCOPE=%q DEPLOY_AGENT_IDLE_TIMEOUT=%q DEPLOY_AGENT_IDLE_INTERVAL=%q DEPLOY_AGENT_IDLE_GUARD_SECONDS=%q' \
  "${REMOTE_PATH}" "${DEPLOY_BRANCH}" "${TARGET_SHA}" "${SOURCE_REMOTE_URL}" "${FETCH_TIMEOUT}" "${TEST_SCOPE}" "${SERVICE_SCOPE}" \
  "${AGENT_IDLE_TIMEOUT}" "${AGENT_IDLE_INTERVAL}" "${AGENT_IDLE_GUARD_SECONDS}")"
